...
def set_guid():
    ...
    guid = (
        custom_action("LoadCorrectProductGuid", args).rstrip().split(b"=")[1].decode()
    )
...
def is_upgrade(current_install):
    ...
    check_version = custom_action(
        ...
        [
            "--installed-version",
            current_install.config["ProductVersion"],
            "--current-version",
            current_version,
        ],
    ).decode()
...
def upgrade(current_install):
    ...
    args = [
        ...
        "/config/StartupConfiguration.ini",
        "--product-name",
        current_install.config["ProductName"],
        "--modules-dir",
        "/data/modules",
        "--db-connectors-dir",
        "/opt/eset/RemoteAdministrator/Server/setup",
        "--current-version",
        current_install.version,
    ]
    ...
    command = [
        ...
        "--skip-cert",
        "--db-user-username",
        current_settings["P_DB_ADMIN_USERNAME"],
        "--db-user-password",
        current_settings["P_DB_ADMIN_PASSWORD"],
        "--db-name",
        current_settings["P_DB_NAME"],
        "--db-hostname",
        current_settings["P_DB_HOSTNAME"],
        "--db-port",
        current_settings["P_DB_PORT"],
        "--db-driver",
        current_settings["P_DB_DRIVER"],
        "--db-type",
        current_settings["P_DB_TYPE"],
    ]
...
def main():
    ...
    subprocess.check_call(["/opt/eset/RemoteAdministrator/Server/ERAServer"])
